...
function user_setup()
    ...
    state.NM = M(false, 'NM')
    state.CP = M(false, "Capacity Points Mode")
    state.WeaponMode = M{['description']='Weapon Mode','Crocea Daybreak','Naegling Machaera', 'Crocea Levante', 'Maxentius Machaera', 'Lvl1Daggers', 'Aeolian Edge'}
...
--	send_command('bind ^z input /ma "Viruna" <stal>')
	send_command('bind ^x input /ma "Poisona" <stal>')
...
function customize_melee_set(meleeSet)
	...
	if state.WeaponMode.Current == 'Crocea Daybreak' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.Magical)
	elseif state.WeaponMode.Current == 'Naegling Machaera' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.Physical)
	elseif state.WeaponMode.Current == 'Crocea Levante' then
	...
	elseif state.WeaponMode.Current == 'Lvl1Daggers' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.Daggers)
    elseif state.WeaponMode.Current == 'Aeolian Edge' then
