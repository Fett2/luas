...
function user_setup()
    ...
    state.WeaponMode = M{['description']='Weapon Mode', 'Naegling', 'Loxotic Mace +1', 'Shining One'}
...
function customize_melee_set(meleeSet)
	...
	if state.TreasureMode.value == 'Fulltime' then
	    ...
	    if state.WeaponMode.Current == 'Naegling' then
	    meleeSet = set_combine(meleeSet, sets.engaged.Naegling)
    elseif state.WeaponMode.Current == 'Chango' then
	...
	elseif state.WeaponMode.Current == 'Loxotic Mace +1' then
	    meleeSet = set_combine(meleeSet, sets.engaged.Loxotic)
	elseif state.WeaponMode.Current == 'Shining One' then
...
function determine_attack_capped()
--	classes.CustomMeleeGroups:clear()
-- send_command('@input /echo <----- variable value: ' .. Attack .. ' ----->')
end
...
function gearinfo(cmdParams, eventArgs)
    if cmdParams[1] == 'gearinfo' then
		...
		if type(tonumber(cmdParams[5])) == 'number' then
            if tonumber(cmdParams[5]) ~= Attack then
                Attack = tonumber(cmdParams[5])
            end
		end
        if not midaction() then
            job_update()
