...
function job_setup()
    ...
    degrade_array = {
		...
		['Fires'] = {'Fire', 'Fire II', 'Fire III', 'Fire IV', 'Fire V'},
		['Waters'] = {'Water', 'Water II', 'Water III', 'Water IV', 'Water V'},
		['Blizzards'] = {'Blizzard', 'Blizzard II', 'Blizzard III', 'Blizzard IV', 'Blizzard V'},
		['Aeros'] = {'Aero', 'Aero II', 'Aero III', 'Aero IV', 'Aero V'},
		['Stones'] = {'Stone', 'Stone II', 'Stone III', 'Stone IV', 'Stone V'},
		['Thunders'] = {'Thunder', 'Thunder II', 'Thunder III', 'Thunder IV', 'Thunder V'},
        }
...
function user_setup()
	...
	state.StormSpell = M{['description']='Storm Spell', 'Sandstorm II', 'Rainstorm II', 'Windstorm II', 'Firestorm II', 'Hailstorm II', 'Thunderstorm II', 'Voidstorm II', 'Aurorastorm II'}
	state.ElementalSpell = M{['description']='Elemental Spell', 'Stone V', 'Water V', 'Aero V', 'Fire V', 'Blizzard V', 'Thunder V'}
	state.HelixSpell = M{['description']='Helix Spell', 'Geohelix II', 'Hydrohelix II', 'Anemohelix II', 'Pyrohelix II', 'Cryohelix II', 'Ionohelix II', 'Noctohelix II', 'Luminohelix II'}
	state.Element = M{['description']='Element', 'Earth', 'Water', 'Air', 'Fire', 'Ice', 'Thunder', 'Dark', 'Light'}
...
--    include('Global-GEO-Binds.lua') -- OK to remove this line
	...
	send_command('bind ^insert gs c cycle StormSpell')
    send_command('bind ^delete gs c cycleback StormSpell')
	send_command('bind @insert gs c stormspell')
	send_command('bind !insert exec elementalforward')
	send_command('bind !delete exec elementabackward')
	send_command('bind ^home gs c cycle Elementalspell')
    send_command('bind ^end gs c cycleback Elementalspell')
	send_command('bind @home gs c elementalspell')
	send_command('bind ^pageup gs c cycle HelixSpell')
    send_command('bind ^pagedown gs c cycleback HelixSpell')
	send_command('bind @pageup gs c helixspell')
	send_command('bind ^k input /ma "Klimaform <me>')
	send_command('bind @delete exec elementalreset')
	...
	send_command('nostrum send Poisson')
	send_command('nostrum p sch')
...
function init_gear_sets()
    ...
    --    /RDM --15
        ...
        body={ name="Merlinic Jubbah", augments={'"Fast Cast"+7','MND+7','"Mag.Atk.Bns."+14',}}, --13
...
--80 without RDM
    ...
    sets.midcast.Cure = {
        ...
        back={ name="Lugh's Cape", augments={'MND+20','Eva.+20 /Mag. Eva.+20','MND+10','"Cure" potency +10%',}}, --10
        waist="Bishop's Sash",
        }
--enmity -52 (62 with cape)
    ...
    sets.midcast.Cursna = set_combine(sets.midcast.StatusRemoval, {
        ...
        feet={ name="Vanya Clogs", augments={'Healing magic skill +20','"Cure" spellcasting time -7%','Magic dmg. taken -3',}}, --5c
        neck="Debilis Medallion", --15c
        ear1="Beatific Earring", --4h
		ear2="Meili Earring", --10h
		rin1="Haoma's Ring", --15c 8h
        ring2="Menelaus's Ring", --20c
        back="Oretan. Cape +1",
        })
    ...
    sets.midcast['Enhancing Magic'] = {
        ...
        back={ name="Lugh's Cape", augments={'INT+20','Mag. Acc+20 /Mag. Dmg.+20','Magic Damage +10','"Mag.Atk.Bns."+10','Phys. dmg. taken-10%',}},
        waist="Olympus Sash",
        }
    ...
    sets.midcast.MndEnfeebles = {
        ...
        ring1="Kishar Ring",
        ring2="Metamor. Ring +1",
        back={ name="Lugh's Cape", augments={'MND+20','Eva.+20 /Mag. Eva.+20','MND+10','"Cure" potency +10%',}},
        waist="Luminary Sash",
        }
...
sets.magic_burst.klimaform = set_combine(sets.magic_burst, sets.buff['Klimaform'])
...
function job_precast(spell, action, spellMap, eventArgs)
	...
	if spell.name:startswith('Water') then
        refine_various_spells(spell, action, spellMap, eventArgs)
    end
	if spell.name:startswith('Blizzard') then
        refine_various_spells(spell, action, spellMap, eventArgs)
    end
	if spell.name:startswith('Aero') then
        refine_various_spells(spell, action, spellMap, eventArgs)
    end
	if spell.name:startswith('Stone') then
        refine_various_spells(spell, action, spellMap, eventArgs)
    end
	if spell.name:startswith('Thunder') then
        refine_various_spells(spell, action, spellMap, eventArgs)
    end
end
...
function job_post_midcast(spell, action, spellMap, eventArgs)
    ...
    if spell.skill == 'Elemental Magic' and state.MagicBurst.value and buffactive['Klimaform'] and spell.element == world.weather_element then
		equip(sets.magic_burst.klimaform)
...
function job_self_command(cmdParams, eventArgs)
	...
	if cmdParams[1]:lower() == 'stormspell' then
        send_command('@input /ma '..state.StormSpell.value..' <t>')
	end
	if cmdParams[1]:lower() == 'elementalspell' then
        send_command('@input /ma '..state.ElementalSpell.value..' <t>')
	end
	if cmdParams[1]:lower() == 'helixspell' then
        send_command('@input /ma '..state.HelixSpell.value..' <t>')
	end
end
...
function refine_various_spells(spell, action, spellMap, eventArgs)
    ...
    if spell_recasts[spell.recast_id] > 0 then
        if spell.name:startswith('Fire') then
            spell_index = table.find(degrade_array['Fires'],spell.name)
            if spell_index > 1 then
                newSpell = degrade_array['Fires'][spell_index - 1]
                send_command('@input /ma '..newSpell..' '..tostring(spell.target.raw))
                eventArgs.cancel = true
            end
        end
    ...
    if spell_recasts[spell.recast_id] > 0 then
        if spell.name:startswith('Water') then
            spell_index = table.find(degrade_array['Waters'],spell.name)
            if spell_index > 1 then
                newSpell = degrade_array['Waters'][spell_index - 1]
                send_command('@input /ma '..newSpell..' '..tostring(spell.target.raw))
                eventArgs.cancel = true
            end
        end
    ...
    if spell_recasts[spell.recast_id] > 0 then
        if spell.name:startswith('Blizzard') then
            spell_index = table.find(degrade_array['Blizzards'],spell.name)
            if spell_index > 1 then
                newSpell = degrade_array['Blizzards'][spell_index - 1]
                send_command('@input /ma '..newSpell..' '..tostring(spell.target.raw))
                eventArgs.cancel = true
            end
        end
    ...
    if spell_recasts[spell.recast_id] > 0 then
        if spell.name:startswith('Aero') then
            spell_index = table.find(degrade_array['Aeros'],spell.name)
            if spell_index > 1 then
                newSpell = degrade_array['Aeros'][spell_index - 1]
                send_command('@input /ma '..newSpell..' '..tostring(spell.target.raw))
                eventArgs.cancel = true
            end
        end
    ...
    if spell_recasts[spell.recast_id] > 0 then
        if spell.name:startswith('Stone') then
            spell_index = table.find(degrade_array['Stones'],spell.name)
            if spell_index > 1 then
                newSpell = degrade_array['Stones'][spell_index - 1]
                send_command('@input /ma '..newSpell..' '..tostring(spell.target.raw))
                eventArgs.cancel = true
            end
        end
    ...
    if spell_recasts[spell.recast_id] > 0 then
        if spell.name:startswith('Thunder') then
            spell_index = table.find(degrade_array['Thunders'],spell.name)
            if spell_index > 1 then
                newSpell = degrade_array['Thunders'][spell_index - 1]
                send_command('@input /ma '..newSpell..' '..tostring(spell.target.raw))
                eventArgs.cancel = true
            end
        end
...
--	if state.Element.Value == 'Earth' then
--	send_command('exec earth.txt')
--	send_command('gs c set ElementalSpell Stone V')
--	end
--end