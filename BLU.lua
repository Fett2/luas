...
function user_setup()
    ...
    state.WeaponMode = M{['description']='Weapon Mode', 'Tizona Machaera', 'Tizona Maxentius', 'Tizona Almace', 'Maxentius Machaera', 'None'}
	state.RedProc = M{['description']='Red Proc', 'None', 'Sword', 'Club', 'Staff', 'Polearm', 'Scythe', 'Great Sword'}
...
--    send_command('bind !] input /ja "Unbridled Learning" <me>')
    ...
    send_command('bind @r gs c cycle RedProc')
	send_command('bind @t gs c cycleback Redproc')
...
--		body=TaeonBody.FC, (Empy body NQ) --13
    ...
    sets.precast.WS['Chant du Cygne'] = set_combine(sets.precast.WS, {
        head={ name="Adhemar Bonnet +1", augments={'DEX+12','AGI+12','Accuracy+20',}}, --0/6
        body="Abnoba Kaftan", --5/5
        hands={ name="Adhemar Wrist. +1", augments={'DEX+12','AGI+12','Accuracy+20',}}, 
        legs="Samnuha Tights",
        feet="Thereoid Greaves", --4/5
        neck="Mirage Stole +2",
        ear1="Odr Earring", --5/0
		ear2="Mache Earring +1",
        ring1="Begrudging Ring",
        ring2="Epona's Ring",
        back={ name = "Rosmerta's Cape", augments= { 'DEX+20','Accuracy+20 Attack+20','DEX+10','Crit.hit rate+10',}}, --10/0
        }) --23/16
...
--        hands="Jhakri Cuffs +2",
 ...
 --       head="Luh. Keffiyeh +3",
    ...
    sets.midcast['Blue Magic'] = {
        ...
        ring2={name="Stikini Ring +1", bag="wardrobe1"}, --8
    ...
    sets.midcast['Blue Magic'].Physical = {
		...
		body = "Assim. Jubbah +3",
    	hands = { name="Herculean Gloves", augments={'Accuracy+11 Attack+11','"Triple Atk."+2','STR+10','Accuracy+15','Attack+5', } },
		ring1="Shukuyu Ring",
		 ring2="Ilabrat Ring",
		back ={ name = "Rosmerta's Cape", augments = { 'STR+20','Accuracy+20 Attack+20','STR+10','Weapon skill damage +10%', } },
		waist="Sailfi Belt +1",
		legs={ name="Herculean Trousers", augments={'Weapon skill damage +3%','STR+15','Accuracy+7','Attack+9',}},
...
--        hands=gear.Adhemar_A_hands,
    ...
    sets.engaged.DW.Machaera = {
	    main="Tizona",
		sub="Machaera +2"
		}
    sets.engaged.DW.Maxentius = {
	    main="Tizona",
		sub="Maxentius"
		}
    sets.engaged.DW.Almace = {
	    main="Tizona",
		sub="Almace"
		}
	sets.engaged.DW.MaxentiusMelee = {
	    main="Maxentius",
	    sub="Machaera +2"
	    }
	sets.engaged.DW.RedSword = {
 		main="Wax Sword",
        sub="Qutrub Knife",
        }
	sets.engaged.DW.RedClub = {
 		main="Ash Club",
        sub="Qutrub Knife",
        }
...
function customize_idle_set(idleSet)
    ...
    if state.WeaponMode.Current == 'Tizona Machaera' then
	    idleSet = set_combine(idleSet, sets.idle.Machaera)
	elseif state.WeaponMode.Current == 'Tizona Maxentius' then
	    idleSet = set_combine(idleSet, sets.idle.Maxentius)
	elseif state.WeaponMode.Current == 'Tizona Almace' then
	    idleSet = set_combine(idleSet, sets.idle.Almace)
	elseif state.WeaponMode.Current == 'Maxentius Machaera' then
	    idleSet = set_combine(idleSet, sets.idle.MaxentiusMelee)
	end
    return idleSet
end
...
function customize_melee_set(meleeSet)
    ...
    if state.RedProc.Current == 'None' and state.WeaponMode.Current == 'Tizona Machaera' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.Machaera)
	elseif state.RedProc.Current == 'None' and state.WeaponMode.Current == 'Tizona Maxentius' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.Maxentius)
	elseif state.RedProc.Current == 'None' and state.WeaponMode.Current == 'Tizona Almace' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.Almace)
	elseif state.RedProc.Current == 'None' and state.WeaponMode.Current == 'Maxentius Machaera' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.MaxentiusMelee)
	elseif state.WeaponMode.Current == 'None' and state.RedProc.Current == 'Sword' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.RedSword)
	elseif state.WeaponMode.Current == 'None' and state.RedProc.Current == 'Club' then
	    meleeSet = set_combine(meleeSet, sets.engaged.DW.RedClub)
